...
export const createRealtimeSync = (args) => {
    ...
    const fetchBlockEventData = async (block) => {
        ...
        if (shouldRequestLogs) {
            ...
            for (const log of logs) {
                ...
                const transaction = block.transactions.find((t) => t.hash === log.transactionHash);
                if (transaction === undefined) {
                    ...
                    if (transaction.transactionIndex !== log.transactionIndex) {
                        throw new Error(`Detected inconsistent '${args.network.name}' RPC responses. 'log.transactionIndex' ${log.transactionIndex} not found in 'block.transactions' ${block.hash}`);
                    }
                }
            }
        }
    ...
    const fetchAndReconcileLatestBlock = async () => {
        try {
            ...
            if (fetchAndReconcileLatestBlockErrorCount * args.network.pollingInterval >
     ...
     * processed serially.
     */
    const reconcileBlock = mutex(async ({ block, endClock, ...rest }) => {
        const latestBlock = getLatestUnfinalizedBlock();
        // We already saw and handled this block. No-op.
        if (latestBlock.hash === block.hash) {
            args.common.logger.trace({
        ...
        try {
            ...
            if (blockWithEventData.logs.length > 0 ||
                ...
                if (blockWithEventData.transactions.length === 1) {
                    ...
                    _text.push(`${blockWithEventData.transactions.length} transactions`);
                }
    ...
    return {
        start(startArgs) {
            ...
            interval = setInterval(fetchAndReconcileLatestBlock, args.network.pollingInterval);
