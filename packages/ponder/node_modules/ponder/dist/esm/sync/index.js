...
import { buildEvents, decodeEvents, syncBlockToInternal, syncLogToInternal, syncTraceToInternal, syncTransactionReceiptToInternal, syncTransactionToInternal, } from "./events.js";
...
export const createSync = async (params) => {
    ...
    const updateHistoricalStatus = ({ events, network, }) => {
    ...
    async function* getEvents() {
        const to = min(getOmnichainCheckpoint({ tag: "finalized" }), getOmnichainCheckpoint({ tag: "end" }));
        const eventGenerators = Array.from(perNetworkSync.entries()).map(([network, { syncProgress, historicalSync }]) => {
            ...
            async function* decodeEventGenerator(eventGenerator) {
            ...
            async function* sortCompletedAndPendingEvents(eventGenerator) {
                for await (const { events, checkpoint } of eventGenerator) {
                    // Sort out any events before the crash recovery checkpoint
                    if (events.length > 0 &&
                        events[0].checkpoint < params.crashRecoveryCheckpoint) {
                        const [, right] = partition(events, (event) => event.checkpoint <= params.crashRecoveryCheckpoint);
                        ...
                        const [left, right] = partition(events, (event) => event.checkpoint <= to);
            ...
            const localEventGenerator = getLocalEventGenerator({
                ...
                to: min(getMultichainCheckpoint({ tag: "finalized", network }), getMultichainCheckpoint({ tag: "end", network })),
                limit: Math.round(params.common.options.syncEventsQuerySize /
                    (params.indexingBuild.networks.length + 1)),
            });
            return sortCompletedAndPendingEvents(decodeEventGenerator(localEventGenerator));
        });
        for await (const { events } of mergeAsyncGeneratorsWithEventOrder(eventGenerators)) {
            params.common.logger.debug({
                service: "sync",
        ...
        switch (event.type) {
            case "block": {
                const events = buildEvents({
                    ...
                    blockData: {
                        ...
                        transactions: event.transactions.map((transaction) => syncTransactionToInternal({ transaction })),
                        transactionReceipts: event.transactionReceipts.map((transactionReceipt) => syncTransactionReceiptToInternal({ transactionReceipt })),
                        traces: event.traces.map((trace) => syncTraceToInternal({
                            trace,
                            block: event.block,
                            transaction: event.transactions.find((t) => t.hash === trace.transactionHash),
                        })),
                    },
                ...
                if (params.ordering === "multichain") {
                    ...
                    params
                        ...
                        events: readyEvents.sort((a, b) => a.checkpoint < b.checkpoint ? -1 : 1),
                        network,
                    })
                        .then(() => {
            ...
            case "finalize": {
                ...
                if (params.ordering === "omnichain" &&
                    ...
                    const chainId = Number(decodeCheckpoint(getOmnichainCheckpoint({ tag: "current" }))
                        .chainId);
                    const network = params.indexingBuild.networks.find((network) => network.chainId === chainId);
            ...
            case "reorg": {
                ...
                let reorgedEvents = 0;
                const isReorgedEvent = ({ chainId, event: { block } }) => {
    ...
    for (const network of params.indexingBuild.networks) {
        seconds[network.name] = {
            start: Number(decodeCheckpoint(getOmnichainCheckpoint({ tag: "start" }))
                .blockTimestamp),
            end: Number(decodeCheckpoint(min(getOmnichainCheckpoint({ tag: "end" }), getOmnichainCheckpoint({ tag: "finalized" }))).blockTimestamp),
            cached: Number(decodeCheckpoint(min(getOmnichainCheckpoint({ tag: "end" }), getOmnichainCheckpoint({ tag: "finalized" }), params.crashRecoveryCheckpoint)).blockTimestamp),
        };
    }
    return {
        getEvents,
            ...
            case "finalize": {
                ...
                await Promise.all([
                    ...
                    syncStore.insertTransactions({
                        transactions: finalizedBlocks.flatMap(({ transactions }) => transactions),
                        chainId: network.chainId,
                    }),
                    ...
                    ...Array.from(childAddresses.entries()).map(([factory, childAddresses]) => syncStore.insertChildAddresses({
                        factory,
                        childAddresses,
                        chainId: network.chainId,
                    })),
...
export async function* getLocalEventGenerator(params) {
    ...
    for await (const syncCursor of bufferAsyncGenerator(params.localSyncGenerator, Number.POSITIVE_INFINITY)) {
        const initialChildAddresses = new Map();
        ...
        while (cursor < Math.min(syncCursor, toBlock)) {
            const { blockData, cursor: queryCursor } = await params.syncStore.getEventBlockData({
            ...
            const endClock = startClock();
            const events = blockData.flatMap((bd) => buildEvents({
