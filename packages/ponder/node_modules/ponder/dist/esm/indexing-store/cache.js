...
import { getTableColumns, getTableName, is, or, } from "drizzle-orm";
        ...
        async prefetch({ events, db }) {
            ...
            for (const [table, tableCache] of cache) {
                for (const key of tableCache.keys()) {
                    if (spillover.get(table).has(key) ||
            ...
            await Promise.all(Array.from(prediction.entries())
                .filter(([, tablePredictions]) => tablePredictions.size > 0)
                .map(async ([table, tablePredictions]) => {
                const conditions = dedupe(Array.from(tablePredictions), ([key]) => key).map(([, prediction]) => getWhereCondition(table, prediction));
