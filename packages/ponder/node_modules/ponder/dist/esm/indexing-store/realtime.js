...
export const createRealtimeIndexingStore = ({ common, schemaBuild: { schema }, database, }) => {
    ...
    return {
        // @ts-ignore
        find: (table, key) => database.retry(async () => {
        ...
        insert(table) {
            return {
                values: (values) => {
                    // @ts-ignore
                    const inner = {
                        onConflictDoNothing: () => database.retry(async () => {
                            common.metrics.ponder_indexing_store_queries_total.inc({
                                table: getTableName(table),
                                method: "insert",
                            });
                        ...
                        onConflictDoUpdate: (valuesU) => database.retry(async () => {
                            common.metrics.ponder_indexing_store_queries_total.inc({
                                table: getTableName(table),
                                method: "insert",
                            });
                            ...
                            if (Array.isArray(values)) {
                                ...
                                for (const value of values) {
                                    ...
                                    if (row === null) {
                                        try {
                                            rows.push(await database.qb.drizzle
                                                .insert(table)
                                                .values(value)
                                                .returning()
                                                .then((res) => res[0]));
                        ...
                        then: (onFulfilled, onRejected) => database
                            .retry(async () => {
                            common.metrics.ponder_indexing_store_queries_total.inc({
                                table: getTableName(table),
                                method: "insert",
        ...
        update(table, key) {
            return {
                set: (values) => database.retry(async () => {
                    common.metrics.ponder_indexing_store_queries_total.inc({
                        table: getTableName(table),
                        method: "update",
                    });
        ...
        // @ts-ignore
        delete: (table, key) => database.retry(async () => {
            common.metrics.ponder_indexing_store_queries_total.inc({
                table: getTableName(table),
                method: "delete",
            });
