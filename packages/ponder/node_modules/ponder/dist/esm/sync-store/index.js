...
import { checksumAddress, hexToNumber, } from "viem";
...
export const createSyncStore = ({ common, database, }) => ({
        ...
        for (let i = 0; i < filters.length; i++) {
            ...
            for (let j = 0; j < fragments.length; j++) {
                ...
                const _query = database.qb.sync
                    ...
                    .as("unnested"))
                    .select([
                    sql `range_agg(unnested.blocks)`.as("merged_blocks"),
                    sql.raw(`'${i}'`).as("filter"),
    ...
    getChildAddresses: ({ factory }) => database.wrap({ method: "getChildAddresses", includeTraceLogs: true }, () => database.qb.sync
        .with("factory_insert", (qb) => qb
        .insertInto("factories")
        .values({ factory })
        .returning("id")
        ...
        .where("factory_addresses.factory_id", "=", sql `(SELECT id FROM factory_insert)`)
        .execute()
        .then((rows) => {
        const result = new Map();
        for (const { address, block_number } of rows) {
            if (result.has(address) === false ||
        ...
        await database.wrap({ method: "insertLogs", includeTraceLogs: true }, async () => {
            ...
            for (let i = 0; i < logs.length; i += batchSize) {
                await database.qb.sync
                    ...
                    .onConflict((oc) => oc.columns(["chain_id", "block_number", "log_index"]).doNothing())
                    .execute();
            }
        });
    },
    ...
    getEventBlockData: async ({ filters, fromBlock, toBlock, chainId, limit }) => database.wrap({ method: "getEventBlockData", includeTraceLogs: true }, async () => {
        ...
        const transferFilters = filters.filter((f) => f.type === "transfer");
        ...
        const blocksQuery = database.qb.sync
            ...
            "timestamp",
            "hash",
            sql `parent_hash`.as("parentHash"),
            sql `logs_bloom`.as("logsBloom"),
            "miner",
            sql `gas_used`.as("gasUsed"),
            sql `gas_limit`.as("gasLimit"),
            sql `base_fee_per_gas`.as("baseFeePerGas"),
            "nonce",
            sql `mix_hash`.as("mixHash"),
            sql `state_root`.as("stateRoot"),
            sql `receipts_root`.as("receiptsRoot"),
            sql `transactions_root`.as("transactionsRoot"),
            sql `sha3_uncles`.as("sha3Uncles"),
            "size",
            "difficulty",
            sql `total_difficulty`.as("totalDifficulty"),
            sql `extra_data`.as("extraData"),
        ])
            .limit(limit);
        const transactionsQuery = database.qb.sync
            ...
            .select([
            sql `block_number`.as("blockNumber"),
            sql `transaction_index`.as("transactionIndex"),
            "hash",
            "from",
            "to",
            "input",
            ...
            "type",
            "gas",
            sql `gas_price`.as("gasPrice"),
            sql `max_fee_per_gas`.as("maxFeePerGas"),
            sql `max_priority_fee_per_gas`.as("maxPriorityFeePerGas"),
            sql `access_list`.as("accessList"),
        ])
            .limit(limit);
        const transactionReceiptsQuery = database.qb.sync
            ...
            "from",
            "to",
            sql `contract_address`.as("contractAddress"),
            sql `logs_bloom`.as("logsBloom"),
            sql `gas_used`.as("gasUsed"),
            sql `cumulative_gas_used`.as("cumulativeGasUsed"),
            sql `effective_gas_price`.as("effectiveGasPrice"),
            "status",
            "type",
        ])
            .limit(limit);
        ...
        const tracesQuery = database.qb.sync
            ...
            sql `gas_used`.as("gasUsed"),
            "error",
            sql `revert_reason`.as("revertReason"),
            "subcalls",
        ])
            .limit(limit);
        ...
        const supremum = Math.min(blocksRows.length < limit
            ...
            : Number(blocksRows[blocksRows.length - 1].number), transactionsRows.length < limit
            ? Number.POSITIVE_INFINITY
            : Number(transactionsRows[transactionsRows.length - 1].blockNumber), transactionReceiptsRows.length < limit
            ? Number.POSITIVE_INFINITY
            : Number(transactionReceiptsRows[transactionReceiptsRows.length - 1]
                .blockNumber), logsRows.length < limit
            ? Number.POSITIVE_INFINITY
            : Number(logsRows[logsRows.length - 1].blockNumber), tracesRows.length < limit
            ? Number.POSITIVE_INFINITY
        ...
        for (const block of blocksRows) {
            ...
            const logs = [];
            const traces = [];
            while (transactionIndex < transactionsRows.length &&
                ...
                if (transaction.type === "0x0") {
                    ...
                    internalTransaction.maxFeePerGas = BigInt(transaction.maxFeePerGas);
                    internalTransaction.maxPriorityFeePerGas = BigInt(transaction.maxPriorityFeePerGas);
            ...
            while (transactionReceiptIndex < transactionReceiptsRows.length &&
                transactionReceiptsRows[transactionReceiptIndex].blockNumber ===
                    block.number) {
                const transactionReceipt = transactionReceiptsRows[transactionReceiptIndex];
                const internalTransactionReceipt = transactionReceipt;
                internalTransactionReceipt.blockNumber = Number(transactionReceipt.blockNumber);
                if (transactionReceipt.contractAddress !== null) {
                    internalTransactionReceipt.contractAddress = checksumAddress(transactionReceipt.contractAddress);
                }
                internalTransactionReceipt.from = checksumAddress(transactionReceipt.from);
                if (transactionReceipt.to !== null) {
                    internalTransactionReceipt.to = checksumAddress(transactionReceipt.to);
                }
                internalTransactionReceipt.gasUsed = BigInt(transactionReceipt.gasUsed);
                internalTransactionReceipt.cumulativeGasUsed = BigInt(transactionReceipt.cumulativeGasUsed);
                internalTransactionReceipt.effectiveGasPrice = BigInt(transactionReceipt.effectiveGasPrice);
            ...
            while (logIndex < logsRows.length &&
            ...
            while (traceIndex < tracesRows.length &&
        ...
        common.metrics.ponder_historical_extract_duration.inc({ step: "format" }, endClock());
        await new Promise(setImmediate);
        let cursor;
        if (Math.max(blocksRows.length, transactionsRows.length, transactionReceiptsRows.length, logsRows.length, tracesRows.length) !== limit) {
            cursor = toBlock;
        }
        else if (blocksRows.length === limit &&
            Math.max(transactionsRows.length, transactionReceiptsRows.length, logsRows.length, tracesRows.length) !== limit) {
...
const addressFilter = (eb, address, column) => {
    // `factory` filtering is handled in-memory
    if (isAddressFactory(address))
        return eb.val(true);
    // @ts-ignore
    if (Array.isArray(address))
        return eb(column, "in", address);
    // @ts-ignore
    if (typeof address === "string")
        return eb(column, "=", address);
    return eb.val(true);
};
const logFilter = (eb, filter) => {
    const conditions = [];
...
// @ts-expect-error
const transactionFilter = (eb, filter) => {
    const conditions = [];
    conditions.push(addressFilter(eb, filter.fromAddress, "from"));
    conditions.push(addressFilter(eb, filter.toAddress, "to"));
    if (filter.fromBlock !== undefined) {
        conditions.push(eb("transactions.block_number", ">=", String(filter.fromBlock)));
    }
    if (filter.toBlock !== undefined) {
        conditions.push(eb("transactions.block_number", "<=", String(filter.toBlock)));
    }
    return eb.and(conditions);
};
const transferFilter = (eb, filter) => {
    const conditions = [];
    conditions.push(addressFilter(eb, filter.fromAddress, "from"));
    conditions.push(addressFilter(eb, filter.toAddress, "to"));
...
const traceFilter = (eb, filter) => {
    ...
    if (filter.functionSelector !== undefined) {
        if (Array.isArray(filter.functionSelector)) {
            conditions.push(eb(sql `substring(traces.input from 1 for 10)`, "in", filter.functionSelector));
        }
        else {
            conditions.push(eb(sql `substring(traces.input from 1 for 10)`, "=", filter.functionSelector));
        }
    }
    if (filter.fromBlock !== undefined) {
        conditions.push(eb("traces.block_number", ">=", String(filter.fromBlock)));
