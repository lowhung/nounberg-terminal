...
export const buildTable = (rows, columns) => {
    ...
    const columnWidths = columns.map((column) => {
        ...
        const maxWidth = Math.max(...formattedRows.map((val) => val.toString().length), column.title.length);
...
export const buildUiLines = (ui) => {
    const { sync, indexing, app, port, hostname } = ui;
    const lines = [];
    lines.push("");
    if (indexing.hasError) {
        lines.push(pc.cyan("Resolve the error and save your changes to reload the server."));
        return lines;
    }
    ...
    if (indexing.events.length === 0) {
        ...
        lines.push(...buildTable(indexing.events, [
    ...
    if (app.eta !== undefined && app.eta !== 0) {
        progressText += ` (${formatEta(app.eta * 1000)} eta)`;
    }
