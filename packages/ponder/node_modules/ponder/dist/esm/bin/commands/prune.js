...
export async function prune({ cliOptions }) {
    ...
    const ponderSchemas = await database.qb.drizzle
        ...
        .from(TABLES)
        .where(inArray(TABLES.table_schema, database.qb.drizzle
    ...
    for (const { value, schema } of result) {
        ...
        const tableCount = ponderSchemas.find((s) => s.schema === schema).tableCount;
    ...
    await database.qb.drizzle.execute(sql.raw(`DROP TABLE IF EXISTS ${tablesToDrop.join(", ")} CASCADE`));
    logger.warn({
        service: "prune",
        msg: `Dropped ${tablesToDrop.length} tables`,
    });
    await database.qb.drizzle.execute(sql.raw(`DROP FUNCTION IF EXISTS ${functionsToDrop.join(", ")} CASCADE`));
    logger.warn({
        service: "prune",
        msg: `Dropped ${functionsToDrop.length} functions`,
    });
    if (schemasToDrop.length > 0) {
        await database.qb.drizzle.execute(sql.raw(`DROP SCHEMA IF EXISTS ${schemasToDrop.join(", ")} CASCADE`));
        logger.warn({
            service: "prune",
