...
import { ZERO_CHECKPOINT_STRING, decodeCheckpoint, } from '../../utils/checkpoint.js';
...
export async function run({ common, preBuild, schemaBuild, indexingBuild, database, onFatalError, onReloadableError, }) {
    ...
    runCodegen({ common });
    const requestQueues = indexingBuild.networks.map((network) => createRequestQueue({
        network,
        common,
        concurrency: Math.floor(common.options.rpcMaxConcurrency / indexingBuild.networks.length),
    }));
    ...
    for (const network of indexingBuild.networks) {
        ...
        common.metrics.ponder_historical_cached_indexing_seconds.set(label, Math.max(sync.seconds[network.name].cached - sync.seconds[network.name].start, 0));
        common.metrics.ponder_historical_completed_indexing_seconds.set(label, 0);
        common.metrics.ponder_indexing_timestamp.set(label, Math.max(sync.seconds[network.name].cached, sync.seconds[network.name].start));
    }
    ...
    for await (const events of recordAsyncGenerator(sync.getEvents(), (params) => {
        ...
        common.metrics.ponder_historical_transform_duration.inc({ step: "prefetch" }, endClock());
        if (events.length > 0) {
            endClock = startClock();
            await database.retry(async () => {
                await database
                    ...
                    for (const eventChunk of eventChunks) {
                        ...
                        for (const network of indexingBuild.networks) {
                            common.metrics.ponder_historical_completed_indexing_seconds.set({ network: network.name }, Math.max(Number(checkpoint.blockTimestamp) -
                                sync.seconds[network.name].start -
                                sync.seconds[network.name].cached, 0));
                            common.metrics.ponder_indexing_timestamp.set({ network: network.name }, Number(checkpoint.blockTimestamp));
                        }
    ...
    for (const network of indexingBuild.networks) {
        const label = { network: network.name };
        common.metrics.ponder_historical_completed_indexing_seconds.set(label, Math.max(sync.seconds[network.name].end -
            sync.seconds[network.name].start -
            sync.seconds[network.name].cached, 0));
        common.metrics.ponder_indexing_timestamp.set({ network: network.name }, sync.seconds[network.name].end);
    }
                ...
                if (event.events.length > 0) {
                    ...
                    for (const { checkpoint, events } of perBlockEvents) {
                        ...
                        if (preBuild.ordering === "multichain") {
                            const network = indexingBuild.networks.find((network) => network.chainId ===
                                Number(decodeCheckpoint(checkpoint).chainId));
                            common.metrics.ponder_indexing_timestamp.set({ network: network.name }, Number(decodeCheckpoint(checkpoint).blockTimestamp));
                        }
                        else {
                            for (const network of indexingBuild.networks) {
                                common.metrics.ponder_indexing_timestamp.set({ network: network.name }, Number(decodeCheckpoint(checkpoint).blockTimestamp));
