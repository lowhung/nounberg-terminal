...
import { Many, One, and, arrayContained, arrayContains, asc, count, createTableRelationsHelpers, desc, eq, extractTablesRelationalConfig, getTableColumns, gt, gte, inArray, is, isNotNull, isNull, like, lt, lte, ne, not, notInArray, notLike, or, } from "drizzle-orm";
    ...
    return ({ table }) => {
        ...
        if (dataLoader === undefined) {
            dataLoader = new DataLoader(async (encodedIds) => {
                ...
                return (decodedRowFragments
                    // Normalize the decoded row fragments
                    .map((fragment) => Object.fromEntries(Object.entries(fragment).map(([col, val]) => {
                    const column = table.columns[col];
                    if (column === undefined) {
                        throw new Error(`Unknown column '${table.tsName}.${col}' used in dataloader row ID fragment`);
                    }
                    return [col, normalizeColumn(column, val, false)];
                })))
                    // Find the database row corresponding to each normalized row fragment
                    .map((fragment) => rows.find((row) => Object.entries(fragment).every(([col, val]) => row[col] === val))));
            }, { maxBatchSize: 1000 });
