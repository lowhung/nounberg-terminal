...
export const createBuild = async ({ common, cliOptions, }) => {
    ...
    const build = {
        ...
        async executeIndexingFunctions() {
            ...
            const executeResults = await Promise.all(files.map(async (file) => ({
            ...
            for (const executeResult of executeResults) {
                if (executeResult.status === "error") {
                    common.logger.error({
                        service: "build",
                        msg: `Error while executing '${path.relative(common.options.rootDir, executeResult.file)}':`,
                        error: executeResult.error,
                    });
                    return executeResult;
                }
        ...
        async executeApi({ indexingBuild, database }) {
            ...
            if (!fs.existsSync(common.options.apiFile)) {
                const error = new BuildError(`API function file not found. Create a file at ${common.options.apiFile}. Read more: https://ponder.sh/docs/query/api-functions`);
            ...
            if (executeResult.status === "error") {
                common.logger.error({
                    service: "build",
                    msg: `Error while executing '${path.relative(common.options.rootDir, common.options.apiFile)}':`,
                    error: executeResult.error,
                });
                return executeResult;
            }
            ...
            if (!(app instanceof Hono || app?.constructor?.name === "Hono")) {
                const error = new BuildError("API function file does not export a Hono instance as the default export. Read more: https://ponder.sh/docs/query/api-functions");
                error.stack = undefined;
                common.logger.error({
                    service: "build",
                    msg: "Failed build",
        ...
        namespaceCompile() {
            if (cliOptions.schema === undefined &&
                ...
                const error = new BuildError("Database schema required. Specify with 'DATABASE_SCHEMA' env var or '--schema' CLI flag. Read more: https://ponder.sh/docs/getting-started/database#database-schema");
                error.stack = undefined;
                common.logger.error({
                    service: "build",
                    msg: "Failed build",
            ...
            for (const route of apiResult.app.routes) {
                if (typeof route.path === "string") {
                    if (route.path === "/ready" ||
                        ...
                        route.path === "/health" ||
                        route.path === "/client") {
                        const error = new BuildError(`Validation failed: API route "${route.path}" is reserved for internal use.`);
                        error.stack = undefined;
                        common.logger.error({
                            service: "build",
                            msg: "Failed build",
        ...
        async startDev({ onReload }) {
            ...
            const onFileChange = async (_file) => {
                ...
                const hasConfigUpdate = invalidated.has(common.options.configFile.replace(/\\/g, "/"));
                const hasSchemaUpdate = invalidated.has(common.options.schemaFile.replace(/\\/g, "/"));
                const hasIndexingUpdate = Array.from(invalidated).some((file) => indexingRegex.test(file) && file !== common.options.apiFile);
                const hasApiUpdate = Array.from(invalidated).some((file) => file === common.options.apiFile);
                ...
                if (hasApiUpdate === true &&
                    ...
                    viteNodeRunner.moduleCache.invalidateDepTree(glob.sync(indexingPattern, {
                        ignore: apiPattern,
                    }));
