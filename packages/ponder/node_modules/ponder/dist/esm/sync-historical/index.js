...
export const createHistoricalSync = async (args) => {
    ...
    const syncLogFilter = async (filter, interval) => {
        ...
        for (let i = 0; i < logs.length; i++) {
            ...
            if (transaction === undefined) {
                ...
                if (transaction.transactionIndex !== log.transactionIndex) {
                    throw new Error(`Detected inconsistent RPC responses. 'log.transactionIndex' ${log.transactionIndex} not found in 'block.transactions' ${block.hash}`);
                }
            }
        }
        ...
        if (shouldGetTransactionReceipt(filter)) {
            const transactionReceipts = await Promise.all(dedupe(blocks, (b) => b.hash).map((block) => {
