...
export async function createServer({ common, database, apiBuild, }) {
    const metricsMiddleware = createMiddleware(async (c, next) => {
        ...
        try {
            ...
            const status = c.res.status >= 200 && c.res.status < 300
                ? "2XX"
            ...
            for (const labels of matchedPathLabels) {
                ...
                common.metrics.ponder_http_server_request_size_bytes.observe({ ...labels, status }, requestSize);
                common.metrics.ponder_http_server_response_size_bytes.observe({ ...labels, status }, responseSize);
                common.metrics.ponder_http_server_request_duration_ms.observe({ ...labels, status }, responseDuration);
            }
        }
    });
    const hono = new Hono()
