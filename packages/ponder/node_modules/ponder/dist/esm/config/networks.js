 ...
 */
export function getFinalityBlockCount({ chainId }) {
...
export async function getRpcUrlsForClient(parameters) {
    ...
    async function getRpcUrlsForTransport(transport) {
        switch (transport.type) {
            ...
            case "webSocket": {
                try {
                    ...
                    const symbol = Object.getOwnPropertySymbols(e).find((symbol) => symbol.toString() === "Symbol(kTarget)");
                    if (!symbol)
                        return [];
            ...
            case "fallback": {
                ...
                const fallbackTransports = transport.transports.map((t) => ({
                    ...t.config,
                    ...t.value,
                }));
                const urls = [];
 ...
 */
export function isRpcUrlPublic(rpcUrl) {
    if (rpcUrl === undefined)
        return true;
    if (!publicRpcUrls) {
        ...
        publicRpcUrls = Object.values(chains).reduce((acc, chain) => {
            ...
            (chain.rpcUrls.default
                .webSocket ?? []).forEach((webSocketRpcUrl) => {
