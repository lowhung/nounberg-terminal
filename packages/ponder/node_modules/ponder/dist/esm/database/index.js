...
import { Kysely, Migrator, PostgresDialect, WithSchemaPlugin, sql as ksql, } from "kysely";
import prometheus from "prom-client";
const VERSION = "1";
export const getPonderMeta = (namespace) => {
    if (namespace === "public") {
...
export const getPonderStatus = (namespace) => {
    if (namespace === "public") {
    ...
    const database = {
        ...
        async migrate({ buildId }) {
            ...
            if (hasPonderMetaTable) {
                await this.wrap({ method: "migrate", includeTraceLogs: true }, () => qb.migrate.transaction().execute(async (tx) => {
            ...
            if (hasPonderMetaTable) {
                await qb.migrate
                    ...
                    .where("key", "=", "status")
                    .execute();
                const version = await qb.migrate
                    .selectFrom("_ponder_meta")
                    .select("value")
                    .where("key", "=", "app")
                    .executeTakeFirst()
...
EXECUTE PROCEDURE "${namespace}".${notification};`));
            ...
            const attempt = () => this.wrap({ method: "migrate", includeTraceLogs: true }, () => qb.drizzle.transaction(async (tx) => {
                ...
                if (previousApp === undefined) {
                    ...
                    for (const table of tables) {
                        await tx.execute(sql.raw(`DROP TABLE IF EXISTS "${namespace}"."${getTableName(table)}" CASCADE`));
                        await tx.execute(sql.raw(`DROP TABLE IF EXISTS "${namespace}"."${getTableName(getReorgTable(table))}" CASCADE`));
                    }
                    for (const enumName of schemaBuild.statements.enums.json) {
                        await tx.execute(sql.raw(`DROP TYPE IF EXISTS "${namespace}"."${enumName.name}"`));
                    }
                    await tx.execute(sql.raw(`TRUNCATE TABLE "${namespace}"."${getTableName(PONDER_STATUS)}" CASCADE`));
                    await createEnums();
                    await createTables();
                    createdTables = true;
                }
        ...
        async recoverCheckpoint() {
            ...
            return this.wrap({ method: "recoverCheckpoint", includeTraceLogs: true }, () => qb.drizzle.transaction(async (tx) => {
                const app = await tx
                    .select({ value: PONDER_META.value })
                    .from(PONDER_META)
                    .where(eq(PONDER_META.key, "app"))
                ...
                if (app.checkpoint === ZERO_CHECKPOINT_STRING) {
                    for (const table of tables) {
                        await tx.execute(sql.raw(`TRUNCATE TABLE "${namespace}"."${getTableName(table)}", "${namespace}"."${getTableName(getReorgTable(table))}" CASCADE`));
                    }
                }
                else {
                    // Update metadata
                    ...
                    // Remove triggers
                    for (const table of tables) {
                        await tx.execute(sql.raw(`DROP TRIGGER IF EXISTS "${getTableNames(table).trigger}" ON "${namespace}"."${getTableName(table)}"`));
                    }
                    // Remove indexes
                    for (const indexStatement of schemaBuild.statements.indexes
                        .json) {
                        await tx.execute(sql.raw(`DROP INDEX IF EXISTS "${namespace}"."${indexStatement.data.name}"`));
                        common.logger.info({
                            service: "database",
...
`));
        ...
        async removeTriggers() {
            await this.wrap({ method: "removeTriggers", includeTraceLogs: true }, async () => {
                for (const table of tables) {
                    await qb.drizzle.execute(sql.raw(`DROP TRIGGER IF EXISTS "${getTableNames(table).trigger}" ON "${namespace}"."${getTableName(table)}"`));
                }
            });
        },
        getStatus() {
        ...
        setStatus(status) {
            return this.wrap({ method: "setStatus" }, async () => {
                await this.qb.drizzle
                    ...
                    .values(Object.entries(status).map(([networkName, value]) => ({
