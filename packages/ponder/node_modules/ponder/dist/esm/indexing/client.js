...
export const createCachedViemClient = ({ common, indexingBuild, requestQueues, syncStore, eventCount, }) => {
    let event = undefined;
    const cache = new Map();
    ...
    return {
        getClient(network) {
            const requestQueue = requestQueues[indexingBuild.networks.findIndex((n) => n === network)];
            return createClient({
        ...
        async prefetch({ events }) {
            ...
            await Promise.all(Array.from(chainRequests.entries()).map(async ([chainId, requests]) => {
                const ni = indexingBuild.networks.findIndex((n) => n.chainId === chainId);
...
export const cachedTransport = ({ common, network, requestQueue, syncStore, cache, }) => ({ chain }) => custom({
